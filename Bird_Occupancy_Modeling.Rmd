...
brd_joineddata_model3 <- brd_joineddata_clean %>%
  ...
  filter(scientificName == bird_species) %>%
  filter(detectionMethod == "calling")
...
env_variable = "observedAirTemp"

measured_by_all_detections <- brd_joineddata_model3 %>%
  group_by(plotID, year) %>%
  summarize(
    observedAirTemp = mean(observedAirTemp, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_wider(
    names_from = year,
...
measured_by_matrix <- valid_plot_years %>%
  left_join(airTemp_by_plot_year, by = c("plotID","year")) %>%
  mutate(measuredBy = replace_na(measuredBy, 'not_detected')) %>%
  pivot_wider(
    names_from = year,
    values_from = measuredBy
  ) %>%
  select(1, sort(names(.)[-1]))

measured_by_matrix <- as.data.frame(measured_by_matrix)
rownames(measured_by_matrix) <- measured_by_matrix$plotID
measured_by_matrix <- measured_by_matrix[ , -1]
...
surveycov <- data.frame(
  measuredBy = as.vector(t(as.matrix(measured_by_matrix)))
  )
...
data <- data.frame(
  measuredBy = birds_pao_model3$survcov$measuredBy
  ) %>%
  filter(!is.na(measuredBy), measuredBy != "not_detected")

ggplot(data, aes(x = measuredBy)) + 
  geom_bar(stat = 'count') +
  labs(x = "Measured By", y = "Number of Surveys Conducted") +
