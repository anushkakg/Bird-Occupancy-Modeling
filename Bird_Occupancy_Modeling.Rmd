...
todo: Plot the cloud cover for each survey.
...
```{r}
coef(birds_occupancy_model3, param = 'psi', prob = 0.95)

data <- cbind(site_cov, birds_occupancy_model3$real$psi) %>% 
  distinct(.keep_all = TRUE) %>%
  arrange(cloudClover)

ggplot(data, aes(x = cloudCover, y = est)) +
  geom_point() + 
  labs(x = "Cloud Cover",
       y = "Prediected Detection") +
  lims(y = c(0, 1))
```
