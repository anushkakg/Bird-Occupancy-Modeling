...
# Filter table by species
bird_species <- "Buteo jamaicensis"

brd_joineddata_model3 <- brd_joineddata_clean %>%
  filter(scientificName == bird_species)

# Get all valid surveys years across sites (so we can fill in NAs)
valid_site_time <- brd_perpoint_clean %>%
  distinct(locationID, timeID
)

# Get detections — only species *actually observed at a site* (so we can fill in 0s)
detections <- brd_joineddata_model3 %>%
  group_by(locationID, timeID
) %>%
  summarize(present = 1, .groups = "drop")

# Join and fill
site_by_time <- valid_site_time %>%
  left_join(detections, by = c("locationID", "timeID")) %>%
  mutate(present = replace_na(present, 0)) %>%
  pivot_wider(
    names_from = timeID,
    values_from = present
...
# Create the detection covariate table: average cloud cover per site per year
cloud_cover_by_site_time <- brd_perpoint_clean %>%
  group_by(locationID, timeID
) %>%
  summarize(
    cloudCover = mean((startCloudCoverPercentage + endCloudCoverPercentage)/2, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_wider(
    names_from = timeID
,
  ...
  tibble::column_to_rownames('locationID') 

# View result
head(cloud_cover_by_site_time)
...
surveycov <- data.frame(
  cloudCover = unlist(cloud_cover_by_site_time)
  )
...
```{r}
newdata <- data.frame(
  cloudCover = seq(from = 0, to = 100, by = 5)
  )
predictions <- predict(birds_occupancy_model3, newdata, 'p')
predictions$cloudCover <- newdata$cloudCover
  
# plot the predictions with ggplot2
ggplot(data = predictions, aes(x = cloudCover, y = est)) +
 geom_line() +
 geom_line(aes(x = cloudCover, y = lower_0.95), linetype = "dashed") +
 geom_line(aes(x = cloudCover, y = upper_0.95), linetype = "dashed") +
 labs(x = "Cloud Cover Percentage", 
