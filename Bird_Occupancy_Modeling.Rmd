...
detection_method_all_surveys <- brd_joineddata_clean %>%
  filter(siteID %in% large_sites) %>%
  group_by(plotID, year) %>%
  summarise(detectionMethod = get_mode(detectionMethod), .groups = "drop")

det_method_matrix <- detection_method_all_surveys %>%
  pivot_wider(
    names_from = year,
...
ggplot(data = data.frame(results), aes(x = rownames(results), y = est)) +
  ...
  lims(y = c(0, 1)) +
  labs(y = "Maximul likelihood estimate",
       x = "Parameter")

data <- data.frame(
  detMethod = birds_pao_model3$survcov$detMethod, 
  detections = as.factor(unlist(birds_pao_model3$det.data))
  )

ggplot(data, aes(x = detMethod, fill = detections)) + 
  geom_bar(stat = 'count') +
