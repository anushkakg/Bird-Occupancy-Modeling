...
```
Looking at the two models, we can briefly look at the AIC values. The AIC for the model with nlcdClass is lower (better), so the principle of parsimony suggests this is the better model: it is the more parsimonous model when considering both fit and number of parameters.
...
ggplot(data, aes(x = elevation, y = est)) +
  ...
  lims(y = c(0, 1), x = c(0, 3100))
```


This shows that Melanerpes carolinus has an occupancy probability (psi) of 80% (i.e., if you go to any of these sites, there is a 80% chance of this bird occupying the site) and a detection probability (p) of 91% (i.e., if you visit a site and the bird occupies it, there is a 91% chance you will detect it)
